...
<!--
その上で、改めて皆さんに質問です。  
Laravelを利用しているプロジェクトでクリーンアーキテクチャを採用してRepositoryパターンを実装する場合に、Eloquentとどう向き合っていますでしょうか？   
...
<!--
以前、私がPHPカンファレンス沖縄で「ActiveRecordパターンの呪縛を学びほぐして挑むクリーンアーキテクチャへの入り口」というタイトルで登壇しました。 
今回のタイトルにも入っている「呪縛」というワードは、ここから来ています。
トークの中で、データベースは単なる永続化の手段であって、ビジネスの関心ごとそのものではない。データソースを中心にした設計ではなく、ドメインモデルを中心にした設計にしましょうとお話しました。  
...
[click] まずはEloquent側の前提です。これはLaravel標準の使い方で、モデルと永続化の責務が密接に結びつけて、1テーブル1モデルで扱う発想です。トランザクションスクリプトを書くには非常に便利です。  
...
<!--
DataMapperとActiveRecordとの違いを図解して説明します。  
ActiveRecordパターンでは、ドメインモデルがそのまま永続化モデルになっていて、insert や update といった永続化に関する責務も持ち合わせています。 
Domain ModelとRow　Data Gatewayの2つのアーキテクチャパターンが組み合わされたものです。  
...
[click] つまり、「永続性の無視（Persistence Ignorance）」を実現したいと考えました。簡単に言うと、ドメインモデルがDBやORMの都合を一切知らなくても良い状態です。これを実現したいと考えました
-->
...
[click]
コンストラクタはビジネス上の必要な情報だけを引数で受け取り、IDや更新日時といったデータベースの永続化に必要となるメタデータは、privateで保持することができます。  
...
<!--
先ほどはモデル側の説明でしたが、これをどうDBに永続化するかというと、このようなXMLで保存や読み出し方法を定義する形となります。  
フィールドの名前とカラムの名前、それに変換する型のtypeを定義する形になります。  
[click] このような独立したXML（またはAttribute）でマッピングでき、これによってドメイン変換の詳細を実装する必要がなくなり、責務を分離することができます。
-->
...
<!--
色々悩みながら、このようなコードで対処しました。  
saveを避けて個別ユースケースに沿ったUPDATE分をDoctrineのDQLを使って実装しました。  
楽観ロックの処理もwhere条件に更新日時を付け加えるなど手探りで実装しようとしました。  
[click] 見てわかるように一気にコード量が増えてしまいました。  
これだとプロパティが増えるたびにDQLを修正する必要があり、またコマンド毎にDQLを量産する形になってしまいます。
-->
...
<!--
Doctrineでは、find で取得したインスタンスを更新することでUPDATEできますが、新しいインスタンスでは管理下にないため、新規エンティティとして永続化される挙動になります。  
...
- <carbon-data-format /> ドメインは immutable にしたい（ステートレス）
- <carbon-change-catalog /> Doctrine は mutable な変更追跡が前提となる（ステートフル）
...
<!--
本質的な原因は、ドメインをステートレスにして不変に保とうとする私たちの設計思想と、ステートフルな変更検知モデルのDoctrineの思想が衝突したことにあります。
この設計思想の乖離を埋めるためにDQLでの個別実装が増加し、コードの単純化という当初の期待に反して、新たな複雑さを生む結果となりました。  
...
逆にイミュータブルでステートレスとすることで、副作用がなく、独立した理解が可能になります。  
...
<!--
諦めが悪い自分はイミュータブルなモデルのままでUnit of Workを使う方法を性懲りもなく検討しました  
[click] 一応、できるにはできます！  
...
<!--
有識者に是非ご意見を頂きたいと思いますが、リポジトリ内でリフレクションを使って、強制的にprivateなプロパティを上書きしてしまうという荒技です。  
...
<!--
Unit of Workの作法に則ってミュータブルを受け入れた所感となります。  
...
<!--
完全なイミュータブルは諦めましたが、良いプラクティスは残したいです。  
...
<div class="flex justify-center items-center gap-12 mt-16">
  ...
  <div class="flex flex-col items-center">
    ...
    <div class="p-4 bg-white rounded-lg shadow-2xl">
      <QRCode value="https://herp.careers/v1/litalico/requisition-groups/ea1f6b9d-88b7-4254-a06a-8cfdd83bd575" :width="200" :height="200" color="000000" image="" />
    </div>
