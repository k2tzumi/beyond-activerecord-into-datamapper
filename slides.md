...
1. Active Record / Eloquent 運用で何がつらかったのか<carbon-face-dissatisfied />   
詰め替えコストとドメイン汚染  
2. Data Mapper / Doctrine に何を期待したのか<carbon-idea />   
POPO と責務分離の魅力  
3. なぜ途中でハマったのか<carbon-warning-alt />  
immutable な設計と Unit of Work の衝突  
4. 最後にどう着地したのか<carbon-checkmark-outline />   
...
<span v-mark.highlight.orange>LLMへ渡すコンテキストを小さく保つための実践知</span>として
改めて注目されている気がしています。
...
<!--
本題に入る前のアイスブレークとして、私が最近感じていることをこのスライドにまとめました。  
[click] クリーンアーキテクチャやレイヤードアーキテクチャは、昔から“きれいな設計”の文脈で語られてきました。  
[click] ただ今はそれだけでなく、LLMに渡すコンテキストを小さく保つ実践として、価値が上がっていると感じています。  
[click] その中で皆さんは大量に生成されるコードに対して、どの様に認知負荷を下げる取り組みをされていますでしょうか？
-->
...
<!--
今回のテーマとしてORMマッパーを扱いますが、コシのある設計の関心ごとがドメインとインフラの分離となります。  
[click] まず関心ごとを分離してデータの永続化という技術的な詳細をモデルから隔離します。  
[click] 次にPOPOにしてインフラ（DB、ネットワーク、ファイルシステム）に依存させないようにしてテストを高速化させたいです。  
[click] 次の変更耐性については、技術的な課題は技術の層だけで完結させます。  
[click] 責務分離というコシが求められる設計をどう実現するか？について今日お話したいと思います。
-->
...
1. EloquentをActiveRecordとして素直に使う<mdi-railroad-light />  
Repositoryは不要。UseCaseからEloquentを直接叩く。Laravelのレールに最も乗った形  
2. EloquentをQueryBuilderとしてのみ使う<carbon-query-queue />  
Eloquentをインフラ層（Repository）に限定し、アプリケーション層へはPOPOなドメインモデルとして渡す  
3. Eloquent自体を捨てる<carbon-trash-can />  
...
おそらく、以下に挙げる3つのパターンのどこかに当てはまるのではないでしょうか。  
[click] 1つ目は素直にActiveRecordとして使うパターン  
[click] 2つ目はQueryBuilderとしてのみ使い、ドメイン層では詰め替えるパターン。  
...
「EloquentをQueryBuilderとして使い、ドメインモデルへ詰め替える」というアプローチ
...
<!--
私のこれまでの立ち位置としては、2番目の「EloquentをQueryBuilderとして使い、DTOへ詰め替える」というアプローチをしていました。  
[click] 読み取り時は、Eloquentモデルからドメインモデルへ   
[click] 書き込み時は、ドメインモデルからEloquentモデルというように、このようにプロパティを一つずつマッピングして詰め直す作業を行っていました。 
-->
...
又、ユースケースが増えるたびに同様の変換が増殖します

<!--
しかし、これが辛いんです。  
カラムが増えるたびに、この変換コードが膨れ上がります。  
本質的なツラミは、ドメインモデルと永続化モデル（Eloquent）が二重管理になってしまい、整合性を保つコストが増え続けることでした。　　
Repositoryにユースケース固有のメソッドが増えるたびに、それぞれで変換コードを書く必要が生じるという構造の問題があります。
-->
...
<carbon-table-split />　ActiveRecordパターンは「1テーブル ＝ 1クラス」という制約を強います<br />  
<carbon-data-base-alt />　「DB設計 ＝ ドメイン設計」という呪縛に陥りがち  

<!--
以前、「ActiveRecordパターンの呪縛を学びほぐして挑むクリーンアーキテクチャへの入り口」というタイトルのスライドを発表したことがあるのですが、  
今回のタイトルの「呪縛」というワードはここから来ています。
データベースは単なる「永続化の手段」であり、ビジネスの「関心事」そのものではない。まずはDBを忘れ、ピュアなビジネスロジックを設計しようというという内容でした。  
今回はこの取り組みからさらに発展した学びを発表します。
-->
...
<!--
[click] 左はEloquent側の前提です。これはLaravel文脈では自然です。  
[click] 右はクリーンアーキテクチャ側の前提です。これも設計として正しい。  
[click] 問題はどちらが正しいかではなく、両者をまたぐ境界で前提がぶつかることです。  
その衝突の副作用として、詰め替えや変換コードが増えていきました。
-->
...
<!--
そこで希望の光となったのが、DataMapperパターンです。
-->
...
<!--
マーティン・ファウラーの定義によると、DataMapperとは「オブジェクトとデータベースの間でデータをやり取りするためのマッパー層」であり、「オブジェクトとデータベースを互いに独立した状態に保つ」ものです。
-->
...
<!--
図解するとこのようなイメージになります。  
ActiveRecordパターンではドメインは永続化モデルになっており、永続化に関する責務を持ち合わせてします。  
[click] これをドメインオブジェクトとDBに間にマッパー層を設けて分離させます。  
...
* <mdi-link-variant /> ドメインモデルと永続化モデルを一つのクラスで扱える効率性。
...
<!--
PHPでDataMapperパターンを実現する際によく使われるのがDoctrineです。   
Doctrineを採用する理由は、ドメインが永続化の関心事から完全に隔離されるからです。  
[click] つまり、「永続性の無視（Persistence Ignorance）」を実現します。簡単にいうとドメインモデルがDBやORMの都合を知らなくてよい状態になります。
-->
...
[click]
純度の高いPOPOとして定義でき、コンストラクタはビジネス上必要な情報だけを受け取ります。  
IDなどの永続化に伴うメタデータはprivateで保持し、ドメイン層からは見えないように隠蔽できます。
-->
...
<!--
そして、ドメインの永続化の知識は、このような独立したXML（またはAttribute）でマッピングします。  
[click] これによって、モデル内に永続化のための手続きを書くことなく、責務を分離できます。
-->
...
<!--
こちらはリポジトリ層のサンプルコードです。最小の実装のイメージですが、非常にシンプルになります。  
[click] プロパティの詰め替えは不要になり、基本的には persist して flush するだけです。登場するモデルは「一つのドメインモデルのみ」となります。
-->
...
* <carbon-data-class /> ドメインモデルとPersistenceモデルを同一クラスで運用しつつ、責務はマッピング層で分離できる

<!--
LaravelでEloquentに親しんできた私からの第一印象としては、「狙い通りの効果はありそうだ」と感じました。  
データの詰め替えはなくなり、ドメインモデルと永続化モデルを同一クラスで運用しつつ、責務はマッピング層で分離できる。
-->
...
<!--
呼び出し側も素直です。findById で取得して、メソッドを呼んで、save する。  
[click]
...
[click] 更新したつもりが。。  
[click]「値引き更新をしたはずなのに、商品レコードが増えていませんか？」と。  
[click] UPDATEしたつもりが、なぜかINSERTされていたんです。
-->
...
<!--
DBを見ると行が増えていたり、一意制約エラーが発生したりしました。
-->
...
<!--
コードレビューで見ても、UPDATEされるケースとINSERTされるケース、どちらも自然なコードに見えます。  
[click] でも結果だけが違う。何が起きているのか分かりません。
-->
...
<!--
おわかり頂けただろうか？。。  
[click] 答え合わせをしますと、UPDATEされるケースは「同じインスタンスをそのまま保存」していました。  
[click] 一方、INSERTされてしまったケースは、メソッドの中で「新しいインスタンスを作って保存」してしまっていたんです。
-->
...
楽観ロックの処理もwhere条件に更新日時に条件を付け加えるなど手探りで実装しようとしていました。  
[click] しかし、これだとプロパティが増えるたびにDQLを修正したり、ユースケースが増えればDQLを量産しなければならず、一気にコード量が増えてしまいました。
-->
...
[click] 楽観的ロックの対応も含めて複雑になり、開発チームからは「もっと楽にできないの？」と悲鳴が上がりました。
-->
...
<!--
なぜこうなったかというと、原因はsaveメソッド側ではありませんでした。  
[click] ドメインモデルの中で、イミュータブル（不変）な実装をしており、メソッドが「新しいインスタンス」を返していたんです。  
[click] Doctrineから見ると、それは「更新対象」ではなく「全く別のオブジェクト」として認識されていました。
-->
...
<!--
Doctrineは、findById で取得したのと同じインスタンスが変更されればUPDATEできますが、新しいインスタンスが渡されると追跡対象から外れ、INSERT側に倒れてしまいます。  
[click] つまり、Doctrineが前提としている変更モデルと、私たちのイミュータブルな設計の相性が悪かったのです。
-->
...
その他にも親子や参照関係を見て、DB制約でこけない順番に自動で SQL を並べたり、  
更新時に version などを条件にして、先に誰かが更新していたら検知して止めたり、  
同じレコードを同一リクエスト内で1つのインスタンスとして扱う機能も含まれていたりします。
...
歴史は繰り返す。DataMapperパターンも設計思想がある
...
<!--
ドメインをイミュータブルに保ちたい設計思想と、変更追跡のためにミュータブルを前提とするDoctrineの思想が衝突しました。  
その溝を埋めるためにDQLや個別更新が増え、期待した単純化の代わりに新たな複雑さを生みました。  
...
<!--
そもそも、なぜイミュータブルにしたいのか。それは「状態変化はバグの温床になる」からです。  
予期せぬ副作用や、メソッドの呼び出し順序への依存などを防ぎ、認知負荷を下げたいという意図がありました。  
状態が変わる設計だと、実行順序を頭の中で追う必要があり、認知負荷が高くなります。  
その結果、関数やクラスを単体で読んでも、挙動を確信しにくくなります。  
DDDでも、Eric EvansはValue Objectのイミュータブル性を重視しています。  
-->
...
この形にしておくと、単なる値の上書きではなく、何が起きた変更かをドメインイベントとして明示しやすくなります。
-->
...
<!--
PHP 8.2から導入された readonly class を使えば、強制的にイミュータブルなモデルにすることができます。  
[click] 言語機能としてこれを強制でき、レビューも楽でかっちりした設計になります。  
[click] ワシワシとした食感が楽しめますw
-->
...
* <carbon-data-view-alt /> リポジトリ側内でfindしてUnit of Workさせる
* <carbon-wikis /> リフレクションでゴニョゴニョさせる

</v-click>

<!--
では、イミュータブルなモデルのままUnit of Workを使う裏技はないのか？と考えました。  
[click] 出来ます！  
[click] リポジトリ内でfindしてリフレクションする方法です
-->
...
<!--
リポジトリ内でリフレクションを使って、強制的にprivateプロパティを書き換えるという荒業も検証しました。  
[click] しかし、これは非公開インターフェースに依存するリスクがあり、変更検知を汎用的に実装するのも難しく、特に集約ルートの更新となると複雑になりすぎるため、筋が悪いと判断しました。
-->
...
<!--
チームで議論した結果、現実的な解決策として「モデルをミュータブルにする」のが最もシンプルで良いという結論に至りました。  
[click] 一部のプロパティを書き換え可能（voidで返すメソッド）にし、ビジネスルールの制約はメソッド内部のガード節で担保するようにしました。
-->
...
<!--
モデルをミュータブルにすることで、Doctrineの機能に乗っかることができます。  
[click] マッピングXMLに version を1行追加するだけで、簡単に楽観的ロックも実現できるようになりました。
-->
...
<!--
リポジトリも再定義していきます。  
[click] リポジトリの実装も見直し、CreateとUpdateを明確に分離しました。  
[click] これによって、EntityRepositoryを継承して基本的なQueryも定義することができ、非常にスッキリしました。  
単純なCRUDは共通化したTraitを利用するのと、EntityRepositoryのQueryも相まって、コード量がかなり削減することができました
-->
...
リポジトリの実装コストは大幅に下がったのと、リポジトリの実装が薄くなり、余分なことが出来なくなったので、責務が明確になった気がします。  
...
* <carbon-badge /> ValueObjectはイミュータブルな実装にする  
...
# <carbon-event /> それでもイミュータブルを諦めたくない
...
<!--  
ここで mutable に寄せれば解決、と言い切れない理由があります。  
[click] ミュータブルにするとUnit of Workとは噛み合いますが、課題も残ります。  
[click] それは「何が起きたか（経緯）」が状態変化の陰に埋もれやすくなることです。  
[click] 値引きなのか、手動補正なのかという意図を見失わない設計が求められます。  
状態の保存ではなく、出来事をドメインイベントとして扱う様に設計していきたいのです。
-->
...
<!--
これはORMの使い方の問題だけでなく、「状態」を主役にする（CRUD）のか、  
[click]「出来事」を主役にする（Event Sourcing）のかというアーキテクチャの分岐点です。  
[click] 一律にどちらかに寄せるのではなく、ケースバイケースで「コシ」のある判断が求められます。  
...
<!--
今回の私たちのプロジェクトでは、現実解としてCRUDを選びました。  
その上で、単純なsetterは禁止する、Value Objectはreadonly必須にするなど、既存のLinterでは防げない部分を、GitHub CopilotなどのAIのインストラクション（指示）を用いてガードレール化する運用でカバーしています。  
[click] 弾力やコシだけでなく、そうしたツールを使った開発プロセス上での滑らかさも重要です。
-->
...
<!--
色々と衝突はありましたが、総論として、ドメインのビジネスルールに集中して向き合える環境を作ることができました。
-->
...
ドメインオブジェクトとDBの間にマッパー層を置き、永続化の都合をドメインから切り離す設計パターン  
「ドメインを守る」ための分離戦略。  
ただし実運用ではUnit of Workの作法まで含めて設計する必要があった  
...
* <carbon-events /> 完全にイミュータブルでUnit of Workにする場合は、イベント駆動に切り替えると良さそう  
Insertだけになるので、Unit of Workの恩恵はそれほどなくなるけれど

<!--
本日のまとめです。  
Data Mapperは、ドメインとDBの間にマッパーを置いて、永続化の都合をドメインから切り離す設計です。  
ポイントは「分離して終わり」ではなく、実運用ではUnit of Workの前提までセットで設計しないと、更新の挙動でつまずきやすいことです。  
Unit of Workは「変更を全部メモしておいて、最後にまとめてSQLを投げる賢い秘書」です。  
DoctrineでEloquentライクな手軽さを求めると摩擦が生じます。  
もし完全にイミュータブルな設計を貫くなら、いっそイベント駆動に切り替える方が相性が良いかもしれません。
-->
...
2. <carbon-view-next /> POPOの公開インターフェースを保ったまま、永続化情報は非公開で保持できる
3. <carbon-scales-tipped /> 完全分離ではなく、責務分離を維持した同居という現実解

<!--
最終的な結論として、「クリーンアーキテクチャにおいては、EloquentよりもDoctrineの方が相性が良い」と感じています。  
完全な分離ではなく、POPOの公開インターフェースを保ちつつ、永続化情報を非公開で同居させる。  
「責務分離を維持した同居」というアプローチが、私たちにとっての現実解でした。
-->
